...
Original file is located at
    https://colab.research.google.com/drive/1kc1LmjmUdCzjLsKu9uq6hjVmcT4TR256
"""
...
def create_and_train_model(data, matrix_type, learning_rate, epochs, batch_size, channels, layers):
  ...
  model = GNN(n_layers=layers, channels=channels)  # Adjust parameters as necessary
  ...
  for batch in loader_tr:
      step += 1
      loss = train_step(*batch)  # Execute the training step
      cumulative_loss += loss.numpy()  # Add the current batch's loss

      # Update and plot the live loss after each batch
      live_plot.update({
          'loss': cumulative_loss / step
      })
      live_plot.send()

      # At the end of each epoch (since the loader already handles epochs)
      if step == loader_tr.steps_per_epoch:
          print("Epoch Loss: {}".format(cumulative_loss / step))
